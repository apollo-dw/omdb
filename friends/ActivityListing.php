<?php
    ...
    if ($reviews) {
        $queries[] = "(
            SELECT
                'review', rv.date, u.UserID, u.Username, rv.ReviewID, rv.SetID,
                CONCAT(bs.Artist, ' - ', bs.Title),
                JSON_OBJECT('Review', LEFT(rv.Comment, 250), 'SetID', bs.SetID)
            FROM user_relations fr
            JOIN users u ON u.UserID = fr.UserIDTo
    ...
    if ($comments) {
        $queries[] = "(
            SELECT
                'comment', c.date, u.UserID, u.Username, c.CommentID, c.SetID,
                CONCAT(bs.Artist, ' - ', bs.Title),
                JSON_OBJECT('Comment', LEFT(c.Comment, 250), 'SetID', bs.SetID)
            FROM user_relations fr
            JOIN users u ON u.UserID = fr.UserIDTo
    ...
    if ($lists) {
        $queries[] = "(
            SELECT
                'list', l.CreatedAt, u.UserID, u.Username, l.ListID, l.ListID, l.Title,
                JSON_OBJECT('Description', LEFT(l.Description, 250))
            FROM user_relations fr
            JOIN users u ON u.UserID = fr.UserIDTo
    ...
    if ($review_likes) {
        $queries[] = "(
            SELECT
                'review_like', rh.CreatedAt, u.UserID, u.Username, rh.HeartID, rv.ReviewID,
                CONCAT(bs.Artist, ' - ', bs.Title),
                JSON_OBJECT('ReviewID', rv.ReviewID, 'SetID', bs.SetID)
            FROM user_relations fr
            JOIN users u ON u.UserID = fr.UserIDTo
    ...
    if ($list_likes) {
        $queries[] = "(
            SELECT
                'list_like', lh.CreatedAt, u.UserID, u.Username, lh.HeartID, l.ListID, l.Title,
                JSON_OBJECT()
            FROM user_relations fr
            JOIN users u ON u.UserID = fr.UserIDTo
    ...
    if ($ranked_maps) {
        $queries[] = "(
            SELECT
                'ranked_map', bs.DateRanked, u.UserID, u.Username, b.BeatmapID, b.BeatmapID,
                CONCAT(bs.Artist, ' - ', bs.Title, ' [', b.DifficultyName, ']'),
                JSON_OBJECT('SetID', bs.SetID)
            FROM user_relations fr
            JOIN users u ON u.UserID = fr.UserIDTo
