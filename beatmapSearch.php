<?php
    ...
    if ($userStmt->num_rows > 0){
        ...
        while ($userStmt->fetch()) {
            $counter += 1;
            ?>
            <a href="/profile/<?php echo $userID; ?>"><div style="<?php if ($counter % 2 == 0){ echo 'background-color:DarkSlateGrey;'; } else { echo 'background-color:#203838;'; } ?>padding:0.25em;display:flex;vertical-align: middle" ><img src="https://s.ppy.sh/a/<?php echo $userID; ?>" style="height:24px;width:24px;" title="<?php echo $username; ?>"/> <?php echo $username; ?></div> </a>
            <?php
        }
    }
