<?php
    ...
    function RenderSimilarMapCards($conn, $similarMaps) {
        if (empty($similarMaps)) {
            echo '<span class="subText recommendation-empty-state">no similar maps found, the difficulty needs at least 10 ratings</span>';
            return;
        }
        ...
        foreach ($similarMaps as $similarMap) {
			...
			<div class="flex-child map-card">
				...
				<span class="subText">
					...
					by <a href="/profile/<?php echo $similarMap["CreatorID"]; ?>"><?php echo safe_htmlspecialchars($similarMapper, ENT_QUOTES); ?></a>
				</span>
			</div>
			<?php
        }
